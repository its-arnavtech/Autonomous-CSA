...
function getSlowQueryThresholdMs() {
  const parsed = Number.parseInt(process.env.DB_SLOW_QUERY_MS?.trim() ?? '250', 10);
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    ...
    this.$on('query' as never, (event: any) => {
      ...
      apiLogger.warn('Slow Prisma query detected', {
